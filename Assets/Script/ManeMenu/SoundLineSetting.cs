...
public class SoundLineSetting : MonoBehaviour
{
	public Image image;
	...
	{
		image.fillAmount = PlayerPrefs.GetFloat("soundVolume"); // При активации окна Setting устанавливаем длину линии в соответствии с сохранением
	...
	private void ChangeFillAmountMusic()
	{
		...
		if (image.type == Image.Type.Filled && image.fillMethod == Image.FillMethod.Horizontal)
		{
			...
			PlayerPrefs.SetFloat("soundVolume", fillAmount);//каждый раз сохраняем громкость звуков
			PlayerPrefs.Save();
			SoundEvents.SetSoundVolume();
			SoundEvents.SetChangeWalkVolume();
			SoundEvents.SetChangeBackVolume();
		}
